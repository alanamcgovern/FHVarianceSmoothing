...
# takes stratified random samples (at the admin1 level)
...
# abline(0,1)
# 
# plot(A_2to1 %*% admin2_means, admin1_means$V1)
# abline(0,1)
...
sample_sizes <- sim_pop %>% group_by(admin1) %>% summarise(N=sum(admin_pop))
...
# simulate samples and direct estimates -------
sample_prop <- 0.00025
#sample_prop <- 0.0005
nsim <- 1000

# assume sample size is roughly proportional to population size (add some noise)
sample_sizes$n <- round(rnorm(nrow(sample_sizes),sample_prop*sample_sizes$N,10))
sample_sizes[sample_sizes$n < 2,]$n <- 2

direct.adm2.estimates <- direct.adm1.estimates <- list()
for(k in 1:nsim){
  cat(k,'\n')
  # sample data using SRS at admin1 level (each unit within an admin1 has equal probability of being sampled) --------
  
  # get indices of samples
  sample_ids <- apply(sample_sizes,1,function(x){sample.int(x['N'],x['n'])})
  #take sample
  sim_sample <- sim_pop_long[, .SD[sample_ids[[admin1]]], by = admin1]
  
  ## get direct estimates -----------
  
  sim_sample$admin1.char <- factor(sim_sample$admin1.char)
  my.svydesign <- survey::svydesign(ids = ~1,strata = ~admin1.char, nest = T, weights = ~1, data = sim_sample)
...
# save(sim_pop_long,file='sim2_popdata.rda')
# save(sample_sizes,file='sim2_samplesizes.rda')
# save(admin2_means,file='sim2_admin2_means.rda')
# save(direct.adm2.estimates,file='sim2_admin2.direct.rda')
# save(direct.adm1.estimates,file='sim2_admin1.direct.rda')
# save(results.adm2,file = 'sim2_admin2.fh.rda')
# save(results.adm1,file = 'sim2_admin1.fh.rda')

#load(file='sim2_popdata.rda')
load(file='sim2_samplesizes.rda')
load(file='sim2_admin2_means.rda')
load(file='sim2_admin2.direct.rda')
load(file='sim2_admin1.direct.rda')
load(file = 'sim2_admin2.fh.rda')
load(file = 'sim2_admin1.fh.rda')
