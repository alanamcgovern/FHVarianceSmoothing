...
for(area in data_areas){
 ...
 # tmp$wt <- pmin(tmp$wt,quantile(tmp$wt,0.95))
  ...
  omega <- tmp$n*tmp$wt
  v_hat_scaled_exact[area] <- dir.dat$variance[area]*sum(tmp$wt^2*tmp$n)
  ...
  q_tmp <- out$values[1:r]
  q_start[area] <- length(q) + 1
  q_id <- c(q_id,rep(area,r))
  q <- c(q,q_tmp)
  
  U <- out$vectors[,1:r]
  nu_main <- c(nu_main,t(U)%*%sqrt(S)%*%rep(1,sum(N)))
  nu_urban <- c(nu_urban, t(U)%*%sqrt(S)%*%tmp$urban)
...
write.csv(data.frame(sim=k,
                     ...
                     lower = apply(theta_fit1,2,quantile,probs=0.05)),file=paste0('Mod2_1/Result',k,'.csv'))
...
write.csv(data.frame(sim=k,
                     ...
                     upper = apply(sig2_fit1,2,quantile,probs=0.95),
                     lower = apply(sig2_fit1,2,quantile,probs=0.05)),file=paste0('Mod2_1/Hyper/Sig2_Result',k,'.csv'))

hyper_fit1 <- as.matrix(unclass(fit1$draws(variables = c("beta",'gamma','sig_u','phi1','df'), format = "matrix")))

write.csv(data.frame(sim=k,
                     model = 'Mod2_1',
                     param = colnames(hyper_fit1),
                     mean = colMeans(hyper_fit1)),
          file=paste0('Mod2_1/Hyper/Hyper_Result',k,'.csv'))
...
write.csv(data.frame(sim=k,
                     ...
                     lower = apply(theta_fit4,2,quantile,probs=0.05)),file=paste0('Mod2_2/Result',k,'.csv'))
...
write.csv(data.frame(sim=k,
                     ...
                     upper = apply(sig2_fit4,2,quantile,probs=0.95),
                     lower = apply(sig2_fit4,2,quantile,probs=0.05)),file=paste0('Mod2_2/Hyper/Sig2_Result',k,'.csv'))

hyper_fit4 <- as.matrix(unclass(fit4$draws(variables = c("beta",'gamma','sig_u','phi1','phi2','df'), format = "matrix")))

write.csv(data.frame(sim=k,
                     model = 'Mod2_2',
                     param = colnames(hyper_fit4),
                     mean = colMeans(hyper_fit4)),
          file=paste0('Mod2_2/Hyper/Hyper_Result',k,'.csv'))
